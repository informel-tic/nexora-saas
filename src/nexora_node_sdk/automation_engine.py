...
# ── Built-in automation templates (shared data — no external imports needed) ──

AUTOMATION_TEMPLATES: dict[str, dict] = {
    "daily_backup": {
        "name": "Daily backup",
        "schedule": "0 2 * * *",
...
def generate_crontab(jobs: list[dict[str, Any]], user: str = "nexora") -> str:
    ...
    lines = [
        f"# Nexora automated jobs — generated {datetime.datetime.now(datetime.timezone.utc).isoformat()}",
        f"# User: {user}",
        "SHELL=/bin/bash",
